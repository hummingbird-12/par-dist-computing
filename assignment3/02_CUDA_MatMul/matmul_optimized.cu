...
__host__ void matmul_optimized(const int BLOCK_DIM, const float mat_a[][MAT_SIZE], const float mat_b[][MAT_SIZE], float mat_c[][MAT_SIZE]) {
    ...
    kernel<<<grid, block, sizeof(float) * BLOCK_DIM * BLOCK_DIM * (2 * GRID_DIM + 1)>>>(dev_a, dev_b, dev_c);
...
__global__ static void kernel(float* mat_a, float* mat_b, float* mat_c) {
    ...
    float* shared_b = shared_a + MAT_SIZE * blockDim.y;     // [MAT_SIZE][blockDim.x]
    float* shared_c = shared_b + MAT_SIZE * blockDim.x;     // [blockDim.y][blockDim.x]
    ...
    for (int i = 0; i < MAT_SIZE; i++) {
        ...
        shared_c[tid] += shared_a[i * blockDim.y + threadIdx.y] * shared_b[i * blockDim.x + threadIdx.x];
    }
