...
__host__ void matmul_shared(const int BLOCK_DIM, const float mat_a[][MAT_SIZE], const float mat_b[][MAT_SIZE], float mat_c[][MAT_SIZE]) {
...
__global__ static void kernel(float* mat_a, float* mat_b, float* mat_c) {
    ...
    float* shared_b = shared_a + blockDim.y * MAT_SIZE;     // [MAT_SIZE][blockDim.x]
    ...
    for (int i = 0; i < MAT_SIZE; i++) {
        ...
        shared_c[tid] += shared_a[threadIdx.y * MAT_SIZE + i] * shared_b[i * blockDim.x + threadIdx.x];
    }
